...
async function create(params: any) {
    if (await db.Account.findOne({ where: { email: params.email } })) {
        throw 'Email "' + params.email + '" is already regsitered';
    }
...
function basicDetails(account: any) {
    const { id, title, firstName, lastName, email, role, created, udpated, isVerified } = account;
    return { id, title, firstName, lastName, email, role, created, udpated, isVerified };
}

async function sendVerificationEmail(accounts: any, origin: any) {
    let message;
    if(origin) {
        const verifyUrl = `${origin}/account/verify-email?token=${accounts.verificationToken}`;
        message = `<p>Please click the below link to verify your email address:</p>
                    <p><a href="${verifyUrl}>${verifyUrl}</a></p>`;
    } else {
        message = `<p>Please use the below token to verify your email address with the <code>/account/verify-email</code> api route:</p>
                    <p><code>${accounts.verificationToken}</code></p>`;
    }
...
async function sendAlreadyRegisteredEmail(email:any, origin: any) {
    ...
    if(origin) {
        ...
        message = `<p>If you don't konw your password you can reset it via the <code>/account/forgot-password</code> api route.</p>`;
    }
    ...
    await sendEmail({
        ...
        html: `<h4>Email Already Registered</h4>
                <p>Your email <strong>${email}</strong> is already regsitered.</p>
                ${message}`
    });
}
